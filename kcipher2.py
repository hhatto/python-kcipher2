#
# implements the KCipher-2 stream cipher
#
...
#
# This code was based on dgryski's go-kcipher2
# https://github.com/dgryski/go-kcipher2
#
...
class KeySizeError(Exception):
    ...
    """kcipher2: invalid key size: %s"""


class IVSizeError(Exception):

    """kcipher2: invalid iv size: %s"""


class KCipher2(object):

    def __init__(self, key, iv):
        """New returns a cipher.Stream implementing the KCipher-2 stream cipher.
        The key and iv parameters must each be 16 bytes.
        """
        self.a = [0 for i in range(5)]
        self.b = [0 for i in range(11)]
        self.iv = [0 for i in range(4)]
        self.ik = [0 for i in range(12)]
        self.sbytes = [0 for i in range(8)]
        self.svalid = 0
        self.l1, self.r1, self.l2, self.r2 = 0, 0, 0, 0

        if len(key) != 32:
            raise KeySizeError(KeySizeError.__doc__ % len(key))

        if len(iv) != 32:
            raise IVSizeError(IVSizeError.__doc__ % len(iv))

        k32 = []
        for i in range(4):
            _tmp_st, _tmp_ed = i * 8, (i + 1) * 8
            if _tmp_ed >= len(key):
                k32.append(int(key[_tmp_st:], 16))
            else:
                k32.append(int(key[_tmp_st:_tmp_ed], 16))

        iv32 = []
        for i in range(4):
            _tmp_st, _tmp_ed = i * 8, (i + 1) * 8
            if _tmp_ed >= len(iv):
                iv32.append(int(iv[_tmp_st:], 16))
            else:
                iv32.append(int(iv[_tmp_st:_tmp_ed], 16))

        self._init(k32, iv32)

    def xor_key_stream(self, src):
        dst = [0 for i in range(len(src))]
        for i in range(len(src)):
            if self.svalid == 0:
                zh, zl = self.stream()
                self.next(MODE_NORMAL)
                _tmp = "%08x" % zh
                for b in range(4):
                    self.sbytes[b] = int(_tmp[b * 2:(b + 1) * 2], 16)
                _tmp = "%08x" % zl
                for b in range(4):
                    self.sbytes[4 + b] = int(_tmp[b * 2:(b + 1) * 2], 16)
                self.svalid = 8
            dst[i] = src[i] ^ self.sbytes[8 - self.svalid]
            self.svalid -= 1
        dst = ["%02X" % i for i in dst]
        return "".join(dst)

    def _key_expansion(self, key, iv):
        ...
        self.ik[3] = key[3]
        # m = 4
        self.ik[4] = self.ik[0] ^ sub_k2(
            ((self.ik[3] << 8) & 0xffffff00) ^ (self.ik[3] >> 24)) ^ 0x01000000

        # m = 4 ... 11, but not 4 nor 8
        self.ik[5] = self.ik[1] ^ self.ik[4]
        self.ik[6] = self.ik[2] ^ self.ik[5]
        self.ik[7] = self.ik[3] ^ self.ik[6]

        # m = 8
        self.ik[8] = self.ik[4] ^ sub_k2(
            (self.ik[7] << 8) ^ (self.ik[7] >> 24)) ^ 0x02000000

        # m = 4 ... 11, but not 4 nor 8
        self.ik[9] = self.ik[5] ^ self.ik[8]
        self.ik[10] = self.ik[6] ^ self.ik[9]
        self.ik[11] = self.ik[7] ^ self.ik[10]

    def _setup_state_values(self, key, iv):
        ...
        """
        # setting up IK and IV by calling _key_expansion(key, iv)
        self._key_expansion(key, iv)
    ...
    def _init(self, key, iv):
        ...
        @modify   S       : (OUTPUT), (A, B, L1, R1, L2, R2)
        """
        self._setup_state_values(key, iv)
        for i in range(24):
            self.next(MODE_INIT)
    ...
    def next(self, m):
        ...
        @modify   S       : (OUTPUT)
        """
        nA = [0 for i in range(5)]
        nB = [0 for i in range(11)]

        nL1 = sub_k2(self.r2 + self.b[4])
        nR1 = sub_k2(self.l2 + self.b[9])
        nL2 = sub_k2(self.l1)
        nR2 = sub_k2(self.r1)
        ...
        # update nA[4]
        temp1 = ((self.a[0] << 8) & 0xffffff00) ^ amul0[
            (self.a[0] >> 24) & 0x000000ff]
        nA[4] = temp1 ^ self.a[3]
        if m == MODE_INIT:
            nA[4] ^= nlf(self.b[0], self.r2, self.r1, self.a[4])

        # update nB[10]
        if self.a[2] & 0x40000000 != 0:   # if A[2][30] == 1
            temp1 = ((self.b[0] << 8) & 0xffffff00) ^ amul1[
                (self.b[0] >> 24) & 0xff]
        else:   # if A[2][30] == 0
            temp1 = ((self.b[0] << 8) & 0xffffff00) ^ amul2[
                (self.b[0] >> 24) & 0xff]

        if self.a[2] & 0x80000000 != 0:   # if A[2][31] == 1
            temp2 = ((self.b[8] << 8) & 0xffffff00) ^ amul3[
                (self.b[8] >> 24) & 0xff]
        else:   # if A[2][31] == 0
            temp2 = self.b[8]

        nB[10] = temp1 ^ self.b[1] ^ self.b[6] ^ temp2

        if m == MODE_INIT:
            nB[10] ^= nlf(self.b[10], self.l2, self.l1, self.a[0])
    ...
    def stream(self):
        ...
        """
        zh = nlf(self.b[10], self.l2, self.l1, self.a[0])
        zl = nlf(self.b[0], self.r2, self.r1, self.a[4])
        return zh, zl
...
def sub_k2(_in):
    ...
    out = int(q3) << 24 | int(q2) << 16 | int(q1) << 8 | int(q0)
